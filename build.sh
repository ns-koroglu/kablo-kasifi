#!/bin/bash
# Kablo Kaşifi — derleme ve .app paketleme betiği
#
#   ./build.sh              → build/Kablo Kaşifi.app üretir
#   ./build.sh --install    → üretir ve /Applications içine kopyalar
#   ./build.sh --run        → üretir ve çalıştırır
#   ./build.sh --reset-perm → (kullanılmıyor)
#
set -euo pipefail
cd "$(dirname "$0")"

APP_NAME="Kablo Kaşifi"
BUNDLE_ID="app.kablokasifi.mac"
EXEC_NAME="KabloKasifi"
OUT_DIR="build"
